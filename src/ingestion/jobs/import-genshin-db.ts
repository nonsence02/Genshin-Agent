...
import { GameDataImportService, type ImportGameDataOptions } from "../services/GameDataImportService.js";

function parseOptions(args: string[]): ImportGameDataOptions {
  const options: ImportGameDataOptions = {};
...
try {
  ...
  service.importAll(options).catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
  });
} catch (error) {
